              ...
              <table className="mt-3 w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                ...
                <tbody>
                  {activeTasks.data.map((activeTask) => (
                    <tr className="bh-white border-b dark:bg-gray-800 dark:border-gray-700" key={activeTask.id}>
                      <td className='px-3 py-2'>{activeTask.id}</td>
                      <th className='px-3 py-2 text-white hover:underline'>
                        <Link href={route("project.show", activeTask.project.id)}>
                      ...
                      <td className='px-3 py-2'>
                        <span className={"px-2 py-1 rounded text-nowrap text-white " + TASK_STATUS_CLASS_MAP[activeTask.status]}>
                          {TASK_STATUS_TEXT_MAP[activeTask.status]}
                        </span>
                      </td>
                      <td className='px-3 py-2'>{activeTask.due_date}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
